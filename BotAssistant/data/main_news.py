...
def get_main_news_ru():
    ...
    page = requests.get(URL)
    soup = BeautifulSoup(page.content, 'html.parser')
    item = soup.find_all('a', class_='focusIndicatorDisplayInlineBlock bbc-1mirykb ecljyjm0')
    item_last = item[1]
    title = item_last.find('span')
    link = 'https://www.bbc.com' + item_last['href']
    title_text = title.get_text(strip=True)
    title_text = title_text.replace('\xa0', ' ')
    object_news = {'title': title_text, 'link': link}
    response.append(object_news)
...
def get_main_news_en():
    ...
    page = requests.get(URL)
    soup = BeautifulSoup(page.content, 'html.parser')
    item = soup.find('a', class_='gs-c-promo-heading gs-o-faux-block-link__overlay-link gel-pica-bold nw-o-link-split__anchor')
    title = soup.find('h3')
    title_text = title.get_text(strip=True)
